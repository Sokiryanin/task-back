...
!!!! ПЕРЕВІР ЩО ЗНАЧЕНННЯ КЛЮЧА ПЕРЕКИНУВ БЕЗ ПРОБІЛІВ ТА З ПАРОЛЕМ !!!!!!!!
у server.js ми беремо з обєкту process.env
const { DB_HOST } = process.env;
Таким чином в хостінг ми передаои додаткові значення які допомогають задеплоїти проєкт.

Щоб сервер запустити локально потрібно в process.env додати DB_HOST:
Встановлюємо пакет dotenv => в корні проєкту створюємо файл .env => записуємо туди DB_HOST = шлях з drivers mongoDB., видаляємо config.js.

В файлі app.js => import dotenv => викликаємо метод config = dotenv.config() (цей пакет дивиться чи є в проєкті текстовий файл .env)
Тепер в глобальному обʼєкті process.env локально такожє рядок DB_HOST із потрібним значенням
...
15. Тепер порібно переробити запити до створеної, так як раніше дані зберігались в JSON.
Mongoose працює за правилом: 
Схема => Модель => Колекція

=> Створюємо модель Board => імпортуємо import { Schema, model } from "mongoose"; => Створюємо мангус- схему - опис того що ми зберігаємо в базі =>  const boardSchema = new Schema({ title: String, tasks: Array }) => створюємо модель const Board = model("boards", boardSchema) в метод model() Першим аргуентом вказуємо назву колекції, другим передаємо створену схему => імпортуємо модель в контроллери. => викликаємо в моделі потрібний метод. 

Методи моделі: 
find() - повертає всю дату;
create() - створює новий обʼєкт в базі, потрібно в нього передавати req.body;
findOne() - повертає обʼєкт по його id;
findById() - повертає обʼєкт по його id;
findByIdAndUpdate() - повертає обʼєкт по його id та оновлює його. Повертає не оновлений обʼєкт. Щоб повертав оновлений треба третім аргументом в запиті передати {new: true} 
findByIdAndDelete() або findByIdAndRemove() - видалення по id

!!! Коли зберігаються нові дані в базу, потрібно обозначити обовʼязкові дані і модель потрібна перевірити дані по схемі !!!

16. Обробка помилок при відправленні не валідних данних у базу:
При кожному додаванні данних в дату додається властивість V - це номер версії даних. Якщо це не потрібно то в схемі другим аргументом потрібно передати {versionKey: false, timestamps: true}

17. Якщо під час додавання відбулася помилка потрібно додати статус 500. Використовуємо просуниті методи роботи с мангусом - Хукі - це ф-ції що відбуваються або до операцій з базою або після. Нам потрібно після невдалого додавання присвоїти статус помилки. boardSchema.post();
Краще робити валідацію JOI - перевірка того що прийшло з бекенда, та валідацю Mongoose - перевірка того що зберігаємо! Всі схеми зберігаємо в одному файлі!

18. Mongodb додає обʼєкт id у кожен створений обʼєкт. І якщо з фронтенда прийде невірне значення, то прокинеться помилка зі статусом 500. Щоб її обробити потрібно створити мідлвар isValidId.js. Викликаємо із монгус метод isValidObjectId та прокидуємо в нього id, якщо не ок то обробляємо помилку. 


19. При оновленні монгус не робить перевірку по своїй схемі. Це потрібно виправляти. Потрібно додати: {new: true, runValidators: true}. При додаванні не валідних значень в новий обʼєкт прийде помилка зі статусом 500. Для цього потрабно додати хук під час оновлення boardSchema.post("findOneAndUpdate", handleSaveError);
Також щоб уникнути проблем, краще зробити преХук перед хуком оновленням. boardSchema.pre("findOneAndUpdate", function (next) {
